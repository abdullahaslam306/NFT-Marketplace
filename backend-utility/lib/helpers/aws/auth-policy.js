...
function AuthPolicy(principal, awsAccountId, apiOptions) {
  ...
  if (!apiOptions || !apiOptions.context) {
    this.context = null;
  } else {
    this.context = apiOptions.context;
  }
}
...
AuthPolicy.prototype = (function AuthPolicyClass() {
  ...
  return {
    ...
    build() {
      ...
      if (this.context && this.context instanceof Object) {
        const localContext = {};
        Object.keys(this.context).forEach(key => {
          if (this.context[key] instanceof Object || this.context[key] instanceof Array) {
            localContext[key] = JSON.stringify(this.context[key]);
          } else {
            localContext[key] = this.context[key];
          }
        });

        policy.context = localContext;
      }
      return policy;
    },
  };
